...
func (h *Handler) ProcessingJob(c *gin.Context) {
	ctx := c.Request.Context()
	var newRequestJob []models.RequestJob
	err := json.NewDecoder(c.Request.Body).Decode(&newRequestJob)
	if err != nil {
		// fmt.Println("]]]]]]]]]]]]]]]]]]]")
		log.Info().Msg("error while converting request body to json")
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"msg2": "decode fail"})
		return

	}
	// fmt.Println("[[[[[[[[[[[[[[[[[[[[[]]]]]]]]]]]]]]]]]]]]]")
	jobData, err := h.service.ProcessingJob(ctx, newRequestJob)
